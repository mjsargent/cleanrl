...
class LevyHead(nn.Module):
    ...
    def forward(self, mu, scale):
        b = mu.shape[0]        
        noise = torch.tensor((norm.ppf(1-np.random.rand(b))**-2, dtype=torch.float, device = mu.device, requires_grad=False))
...
class Actor(nn.Module):
    def __init__(self, env):
        ...
        self.levy_mu_head = nn.Sequential(nn.Linear(256, 128), nn.ReLU(), nn.Linear(128,1), nn.ReLU())
        
        self.levy_scale_head = nn.Sequential(nn.Linear(256, 128), nn.ReLU(), nn.Linear(128,1), nn.ReLU())
    ...
    def forward(self, x, device):
        ...
        z = F.relu(self.fc2(x))
        z_n = z.clone().detach()
        levy_mu = self.levy_mu_head(z_n)
        levy_scale = self.levy_scale_head(z_n)
        n = self.LevyHead(levy_mu, levy_scale) 

        return torch.tanh(self.fc_mu(z)), n
