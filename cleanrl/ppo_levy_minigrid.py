...
if __name__ == "__main__":
    ...
    parser.add_argument('--gym_id', type=str, default="CartPole-v0",
...
class SampleFromLevy(nn.Module):
    def __init__(self):
        super(SampleFromLevy,self).__init__()
    ...
    def forward(self, mu,scale):
        ...
        scale = torch.clamp(scale,min=0.000001,max=1) 
        n = mu+scale*noise 
        n = torch.clamp(n, 0, 25.0001) 
        return n
...
class Agent(nn.Module):
    def __init__(self, envs, frames=3):
        ...
        #m = envs[0].observation_space.shape[1]
        n = 7
        m = 7
        ...
        self.policy_repeat_non_linearity = lambda x: nn.ReLU().forward(x-1)
        self.policy_repeat_sampler = SampleFromLevy()
    ...
    def repeat_policy(self,prev_pol, prev_n, current_pol, current_n ):
       # print("non llin:", self.policy_repeat_non_linearity(1.999999-prev_n))
       # print("1/ term", (1.999999 - prev_n))
        new_n = self.policy_repeat_non_linearity(prev_n) + \
            (self.policy_repeat_non_linearity(1.999999 - prev_n ))*current_n/(0.999999 - prev_n) 
        new_pol = self.policy_repeat_non_linearity(prev_n)*prev_pol/(prev_n-1) + \
            (self.policy_repeat_non_linearity(1.999999 - prev_n))*current_pol/(0.999999 - prev_n)
        return new_pol, new_n
    ...
    def get_mu(self,x):
        mu = self.mu_head(self.forward(x))
        return mu

    def get_scale(self,x):
        scale= self.scale_head(self.forward(x))
        return scale
    ...
    for step in range(0, args.num_steps):
        ...
        for info in infos:
            if 'episode' in info.keys():
                ...
                writer.add_scalar("charts/last_n",float( prev_n[0]) ,global_step)
                writer.add_scalar("charts/last_mu",float( mu[0]) ,global_step)
                writer.add_scalar("charts/last_scale",float(scale[0]) ,global_step)
                prev_n = None
                prev_pol = None
                break
